...
// V5 — reveal the real sticky header only after the exact approved hero moves away.
(function(){
  const hero = document.querySelector(".hero-exact");
  const topbar = document.querySelector(".topbar-after-hero");
  ...
  function syncHeroHeader(){
    ...
    const trigger = Math.max(80, hero.offsetHeight * 0.72);
    topbar.classList.toggle("past-hero", scrollY > trigger);
  }
